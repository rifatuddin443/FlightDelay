    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            time_in = int(self.seq_len) * int(self.stage3_time_feature_dim)
            self.stage3_time_embed = nn.Sequential(
                nn.Flatten(start_dim=1),
                nn.Linear(time_in, self.hidden_channels),
                nn.GELU(),
                nn.Dropout(p=0.1),
                nn.Linear(self.hidden_channels, self.hidden_channels),
            )
            print(
                f"[Stage 3] Time features enabled (dim={self.stage3_time_feature_dim}, seq_len={self.seq_len}, flat={time_in})"
            )
