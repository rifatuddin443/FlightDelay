...
def _neighbor_mean_features(x: torch.Tensor, edge_index: torch.Tensor, *, chunk_size: int = 256) -> torch.Tensor:
    """Compute mean neighbor features using a PyG-style edge_index.

    IMPORTANT: This is implemented via chunked sparse-matrix multiplication to
    avoid creating an intermediate tensor of shape [S, E, ...] (which can be
    enormous for large S).

    Supports x shaped [S, N, ...] or [N, ...]. Aggregation is src->dst.
    """
    if edge_index is None:
        return torch.zeros_like(x)

    if x.dim() == 1:
        raise ValueError(f"Expected x with node dimension, got shape {tuple(x.shape)}")
    ...
    if edge_index.numel() == 0:
        out = torch.zeros_like(x_b)
        return out.squeeze(0) if added_batch else out

    if edge_index.dtype != torch.long:
        edge_index = edge_index.long()

    num_nodes = int(x_b.shape[1])
    src = edge_index[0].clamp(0, num_nodes - 1)
    dst = edge_index[1].clamp(0, num_nodes - 1)

    # Build row-normalized sparse adjacency A_norm where A_norm[dst, src] = 1/deg(dst).
    values = torch.ones(dst.numel(), device=x_b.device, dtype=x_b.dtype)
    deg = torch.zeros(num_nodes, device=x_b.device, dtype=x_b.dtype)
    deg.index_add_(0, dst, values)
    inv_deg = deg.clamp_min(1.0).reciprocal()
    values = values * inv_deg.index_select(0, dst)

    indices = torch.stack([dst, src], dim=0)
    adj_norm = torch.sparse_coo_tensor(indices, values, (num_nodes, num_nodes)).coalesce()

    # Flatten trailing dims for efficient sparse mm.
    orig_shape = x_b.shape
    s_len = int(orig_shape[0])
    trailing = int(torch.tensor(orig_shape[2:]).prod().item()) if len(orig_shape) > 2 else 1

    out = torch.empty_like(x_b)

    # Chunk along sample dimension to keep memory bounded.
    step = max(1, int(chunk_size))
    for start in range(0, s_len, step):
        end = min(s_len, start + step)
        xb = x_b[start:end]
        bsz = int(xb.shape[0])
        xb_flat = xb.reshape(bsz, num_nodes, trailing)
        # [N, B*F]
        mat = xb_flat.permute(1, 0, 2).reshape(num_nodes, bsz * trailing)
        out_mat = torch.sparse.mm(adj_norm, mat)
        out_flat = out_mat.reshape(num_nodes, bsz, trailing).permute(1, 0, 2)
        out[start:end] = out_flat.reshape((bsz, num_nodes, *orig_shape[2:]))

    return out.squeeze(0) if added_batch else out


def _mix_three_adjacencies(
    x: torch.Tensor,
    edge_index_adj: torch.Tensor,
    edge_index_od: torch.Tensor,
    edge_index_od_t: torch.Tensor,
    *,
    ...
    """Incorporate three adjacency matrices by mixing neighbor means into x.

    Keeps the original feature dimensionality (so the TCN encoder setup stays unchanged).
    """
    x_out = x
    if w_adj != 0.0:
        t0 = time.time()
        x_out = x_out + float(w_adj) * _neighbor_mean_features(x_out, edge_index_adj)
        print(f"    ✓ mixed edge_index_adj in {time.time() - t0:.2f}s", flush=True)
    if w_od != 0.0:
        t0 = time.time()
        x_out = x_out + float(w_od) * _neighbor_mean_features(x_out, edge_index_od)
        print(f"    ✓ mixed edge_index_od in {time.time() - t0:.2f}s", flush=True)
    if w_od_t != 0.0:
        t0 = time.time()
        x_out = x_out + float(w_od_t) * _neighbor_mean_features(x_out, edge_index_od_t)
        print(f"    ✓ mixed edge_index_od_t in {time.time() - t0:.2f}s", flush=True)
    return x_out
...
class SequentialTwoStagePredictor(nn.Module):
    ...
    def forward_regressor(self, hidden: torch.Tensor, *, which: str = "delayed") -> torch.Tensor:
...
def train_stage3_opacus(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_x, _, batch_y_reg in train_loader:
            ...
            optimizer.zero_grad(set_to_none=True)
            hidden = base._encode_x(batch_x)
            preds = base.forward_regressor(hidden, which="nondelayed")
...
def parse_args() -> argparse.Namespace:
    ...
    # Adjacency mixing (keeps TCN; injects graph structure into node features pre-encoder)
    parser.add_argument(
        '--adj_mix_adj',
        type=float,
        default=0.2,
        help='Weight for neighbor-mean mixing from adjacency matrix (edge_index_adj). 0 disables.',
    )
    parser.add_argument(
        '--adj_mix_od',
        type=float,
        default=0.2,
        help='Weight for neighbor-mean mixing from OD graph (edge_index_od). 0 disables.',
    )
    parser.add_argument(
        '--adj_mix_od_t',
        type=float,
        default=0.2,
        help='Weight for neighbor-mean mixing from transposed OD graph (edge_index_od_t). 0 disables.',
    )
    parser.add_argument(
        '--epsilonfixed',
        dest='epsilonfixed',
...
def main() -> None:
    ...
    # Keep edge indices on CPU for feature augmentation; move to device for evaluation later.
    edge_index_adj = edge_index_adj.long().cpu()
    edge_index_od = edge_index_od.long().cpu()
    edge_index_od_t = edge_index_od_t.long().cpu()

    if any(float(w) != 0.0 for w in (args.adj_mix_adj, args.adj_mix_od, args.adj_mix_od_t)):
        print("\n[GRAPH INPUT AUGMENTATION] Mixing neighbor features from 3 adjacencies:")
        print(f"  adj_mix_adj={args.adj_mix_adj}, adj_mix_od={args.adj_mix_od}, adj_mix_od_t={args.adj_mix_od_t}")
        t0 = time.time()
        train_x = _mix_three_adjacencies(
            train_x,
            edge_index_adj,
            edge_index_od,
            edge_index_od_t,
            w_adj=float(args.adj_mix_adj),
            w_od=float(args.adj_mix_od),
            w_od_t=float(args.adj_mix_od_t),
        )
        val_x = _mix_three_adjacencies(
            val_x,
            edge_index_adj,
            edge_index_od,
            edge_index_od_t,
        ...
        print("\n[GRAPH INPUT AUGMENTATION] Disabled (all weights are 0.0)")
