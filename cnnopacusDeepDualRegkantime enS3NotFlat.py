...
CHANGE LOG (for edits made in VS Code on this account)
- 2026-01-31: Keep Stage 3 time features sequence-shaped (Conv1d embed), avoid flattening time into T*F.
...
- Uses a CNN encoder (Conv1d over time) with two heads:
 ...
 # Fully-connected classifier head (ends in FC).
        # Deeper MLP: hidden -> hidden -> hidden -> hidden/2 -> out_channels
        # Uses GELU + Dropout(0.2) between layers to stabilize under DP noise.
use kan stage 3 regressor
...
# Fast per-sample gradients (PyTorch 2.x)
try:
    from torch.func import functional_call, vmap, value_and_grad  # type: ignore
    TORCHFUNC_AVAILABLE = True
except Exception:
    functional_call = None  # type: ignore
    vmap = None  # type: ignore
    value_and_grad = None  # type: ignore
    TORCHFUNC_AVAILABLE = False

# KAN head (used optionally for Stage 3 regressor)
try:
    from kan import KAN  # type: ignore
    KAN_AVAILABLE = True
except Exception:
    KAN = None  # type: ignore[assignment]
    KAN_AVAILABLE = False
...
# Architecture tag for output filenames (signals explicit dual-regressor design)
ARCH_TAG = "dualreg"
...
def _run_tag(*, train_script: str, data_source: str, noise_multiplier: float, dp_enabled: bool) -> str:
    return f"{_script_stem(train_script)}_{data_source}_{_sigma_tag(noise_multiplier, dp_enabled)}_{ARCH_TAG}"
...
def _ensure_node_features_s_n_f_t(x: torch.Tensor, seq_len: int) -> torch.Tensor:
    """Ensure node features are shaped [S, N, F, T].

    Supports common inputs:
      - [S, N, T*F] (legacy flattened history)
      - [S, N, T, F]
      - [S, N, F, T]
    """
    if x.dim() == 4:
    ...
    s, n, d = x.shape
    if d % int(seq_len) != 0:
        raise ValueError(f"Cannot reshape x with last_dim={d} into seq_len={seq_len}")
    f = d // int(seq_len)
    x_t_f = x.view(s, n, int(seq_len), f)
    return x_t_f.permute(0, 1, 3, 2).contiguous()
...
class SequentialTwoStagePredictor(nn.Module):
    ...
    architecture can be changed without editing other files.
        ...
        Expected input: `data.x` shaped either
            - [num_nodes, feature_dim, seq_len] (preferred), or
            - [num_nodes, seq_len, feature_dim].

        Legacy support:
            - [num_nodes, seq_len * feature_dim] (will be reshaped if `seq_len` is set).
    ...
    def __init__(
        ...
        *,
        stage3_regressor: str = "mlp",
        kan_grid_size: int = 3,
        kan_spline_order: int = 2,
        stage3_use_time_features: bool = False,
        stage3_time_feature_dim: int = 2,
    ) -> None:
        super().__init__()
        ...
        # In this script, `in_channels` is the per-timestep feature dimension F.
        self.feature_dim: int = self.in_channels

        print(
            f"[SequentialTwoStagePredictor] in_channels(F)={self.in_channels}, seq_len={self.seq_len}, "
            f"feature_dim(F)={self.feature_dim}"
        )

        c1 = max(16, self.hidden_channels // 2)
        c2 = max(16, self.hidden_channels)

        print(f"[Encoder] Using TCN encoder (sequence-aware: [N, F, T])")
        tcn = TemporalConvNet(
            self.feature_dim,
        ...
        self.stage3_use_time_features = bool(stage3_use_time_features)
        self.stage3_time_feature_dim = int(stage3_time_feature_dim)
        self.stage3_time_embed: Optional[nn.Module] = None
        if (
            self.stage3_use_time_features
            and self.stage3_time_feature_dim > 0
        ):
            # Sequence-aware embed: expects time_features shaped [B, D, T] (or [B, T, D]).
            # This avoids flattening time into a single (T*D) vector.
            self.stage3_time_embed = nn.Sequential(
                nn.Conv1d(self.stage3_time_feature_dim, self.hidden_channels, kernel_size=3, padding=1),
            ...
            print(
                f"[Stage 3] Time features enabled (dim={self.stage3_time_feature_dim}, mode=conv1d_pool)"
            )

        # Fully-connected classifier head (ends in FC).
        # Deeper MLP: hidden -> hidden -> hidden -> hidden/2 -> out_channels
        # Uses GELU + Dropout(0.2) between layers to stabilize under DP noise.
        cls_hidden_half = max(8, self.hidden_channels // 2)
        self.classifier = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(p=0.2),
            nn.Linear(self.hidden_channels, cls_hidden_half),
            nn.GELU(),
            nn.Dropout(p=0.2),
            nn.Linear(cls_hidden_half, self.out_channels),
        )

        # Explicit dual-regressor design (permanent modules).
        # - regressor_delayed: trained in Stage 2
        # - regressor_nondelayed: trained in Stage 3
        # Both are 2-3 layers deep (controlled by regressor_extra_layer).
        reg_hidden_half = max(8, self.hidden_channels // 2)
        if regressor_extra_layer:
            reg_block = lambda: nn.Sequential(
                nn.Linear(self.hidden_channels, self.hidden_channels),
                nn.GELU(),
                nn.Dropout(p=0.1),
                nn.Linear(self.hidden_channels, reg_hidden_half),
                nn.GELU(),
                nn.Dropout(p=0.1),
                nn.Linear(reg_hidden_half, self.out_channels),
            )
        else:
            reg_block = lambda: nn.Sequential(
                nn.Linear(self.hidden_channels, reg_hidden_half),
        ...
        self.regressor_delayed = reg_block()
        stage3_regressor = str(stage3_regressor).lower().strip()
        if stage3_regressor in {"kan", "efficient-kan", "efficient_kan"}:
            if not KAN_AVAILABLE or KAN is None:
                raise ImportError(
                    "Stage 3 regressor requested as KAN, but 'kan.py' could not be imported. "
                    "Ensure kan.py is present in the project root."
                )
            # KAN head for the non-delayed regressor (Stage 3)
            self.regressor_nondelayed = KAN(
                layers_hidden=[self.hidden_channels, reg_hidden_half, self.out_channels],
                grid_size=int(kan_grid_size),
                spline_order=int(kan_spline_order),
            )
            print(
                f"[Stage 3 Head] Using KAN regressor_nondelayed "
                f"(grid_size={int(kan_grid_size)}, spline_order={int(kan_spline_order)})"
            )
        else:
            self.regressor_nondelayed = reg_block()
    ...
    def _encode_x(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Alternate: [N, T, F]
        if x.dim() == 3 and x.shape[-1] == self.feature_dim:
            return self.encoder(x.permute(0, 2, 1).contiguous())

        # Legacy: [N, T*F]
        if x.dim() == 2 and self.seq_len is not None and self.seq_len > 0:
            if x.shape[1] == self.seq_len * self.feature_dim:
                x3 = x.view(x.shape[0], self.seq_len, self.feature_dim)
                return self.encoder(x3.permute(0, 2, 1).contiguous())

        raise ValueError(
            f"Unexpected input shape for encoder: {tuple(x.shape)}. "
            f"Expected [N,F,T] or [N,T,F] (or [N,T*F] with seq_len set)."
        )
    ...
    def forward_regressor(
        ...
        *,
        ...
        hidden_in = hidden
        if which in {"nondelayed", "non_delayed", "non-delayed"}:
            if self.stage3_time_embed is not None and time_features is not None:
                tf = time_features
                # Accept [B, D, T] (preferred) or [B, T, D].
                if tf.dim() == 3:
                    if tf.shape[1] == self.stage3_time_feature_dim:
                        pass
                    elif tf.shape[-1] == self.stage3_time_feature_dim:
                        tf = tf.permute(0, 2, 1).contiguous()
                # Backward-compat: accept flattened [B, T*D] if seq_len is known.
                elif tf.dim() == 2 and self.seq_len is not None and self.seq_len > 0:
                    expected = int(self.seq_len) * int(self.stage3_time_feature_dim)
                    if tf.shape[1] == expected:
                        tf = tf.view(tf.shape[0], int(self.seq_len), int(self.stage3_time_feature_dim)).permute(0, 2, 1).contiguous()
                hidden_in = hidden_in + self.stage3_time_embed(tf)

        hidden_dropped = self.dropout_reg(hidden_in)
        if which == "delayed":
            return self.regressor_delayed(hidden_dropped)
        if which in {"nondelayed", "non_delayed", "non-delayed"}:
            return self.regressor_nondelayed(hidden_dropped)
        raise ValueError(f"Unknown regressor selector: {which!r}")

    def forward(self, data: Data) -> Tuple[torch.Tensor, torch.Tensor]:
        hidden = self._encode_x(data.x)
        node_reg = self.forward_regressor(hidden, which="delayed")
...
def _get_accountant(privacy_engine: Optional[object]) -> Optional[object]:
    if privacy_engine is None:
        return None
    return getattr(privacy_engine, "accountant", None)


def _will_exceed_epsilon_next_step(
    accountant: object,
    *,
    ...
    """Return (would_exceed, eps_now, eps_next).

    Uses a deepcopy of the accountant to simulate one more step.
    """
    try:
        eps_now = float(accountant.get_epsilon(float(target_delta)))
    except Exception:
        eps_now = float("nan")

    try:
        acc_copy = copy.deepcopy(accountant)
        acc_copy.step(noise_multiplier=float(noise_multiplier), sample_rate=float(sample_rate))
        eps_next = float(acc_copy.get_epsilon(float(target_delta)))
        return (eps_next > float(target_epsilon)), eps_now, eps_next
    except Exception:
        return False, eps_now, float("nan")


def _manual_dp_sgd_step(
    *,
    loss_fn,
    ...
    """Manual DP-SGD update for Stage 3 modules only.

    - Computes per-sample grads for the non-delayed head (+ optional time embed).
    - Clips per-sample grads to max_grad_norm.
    - Adds Gaussian noise with std = noise_multiplier * max_grad_norm.
    - Applies optimizer step.

    Returns average unclipped loss over samples that were updated.
    """
    base = _unwrap_opacus_model(model)

    stage3_params: List[torch.nn.Parameter] = []
    stage3_params.extend(list(base.regressor_nondelayed.parameters()))
    if getattr(base, "stage3_time_embed", None) is not None:
        stage3_params.extend(list(base.stage3_time_embed.parameters()))
    stage3_params = [p for p in stage3_params if p.requires_grad]
    if not stage3_params:
        return 0.0

    batch_size = int(hidden.shape[0])
    device = hidden.device

    summed_clipped: List[torch.Tensor] = [torch.zeros_like(p, device=device) for p in stage3_params]
    effective_count = 0
    loss_accum = 0.0

    for i in range(batch_size):
        m_i = element_mask[i]
        if float(m_i.sum().item()) <= 0.0:
            continue

        h_i = hidden[i : i + 1]
        t_i = time_features[i : i + 1] if time_features is not None else None
        y_i = targets[i : i + 1]
        m_i = m_i.unsqueeze(0)

        preds_i = base.forward_regressor(h_i, which="nondelayed", time_features=t_i)
        per_elem = loss_fn(preds_i, y_i) * m_i
        denom = m_i.sum(dim=1).clamp_min(1.0)
        loss_i = (per_elem.sum(dim=1) / denom).mean()

        grads = torch.autograd.grad(loss_i, stage3_params, retain_graph=True, allow_unused=False)
        ...
        clip_coef = min(1.0, float(max_grad_norm) / float(total_norm.item() + 1e-12))

        for j, g in enumerate(grads):
            summed_clipped[j] = summed_clipped[j] + g.detach() * clip_coef

        effective_count += 1
        loss_accum += float(loss_i.item())
    ...
    optimizer.zero_grad(set_to_none=True)
    for p, s in zip(stage3_params, summed_clipped):
        noise = torch.randn_like(s) * std
        p.grad = (s + noise) / float(batch_size)
    optimizer.step()
    return loss_accum / float(effective_count)


class _Stage3HeadWrapper(nn.Module):
    def __init__(self, *, regressor_nondelayed: nn.Module, stage3_time_embed: Optional[nn.Module]):
        super().__init__()
        self.regressor_nondelayed = regressor_nondelayed
        self.stage3_time_embed = stage3_time_embed

    def forward(self, hidden: torch.Tensor, time_features: Optional[torch.Tensor] = None) -> torch.Tensor:
        hidden_in = hidden
        if self.stage3_time_embed is not None and time_features is not None:
            hidden_in = hidden_in + self.stage3_time_embed(time_features)
        # IMPORTANT: Avoid dropout here to keep vmap deterministic and fast.
        return self.regressor_nondelayed(hidden_in)


def _manual_dp_sgd_step_vmap(
    *,
    loss_fn,
    model: SequentialTwoStagePredictor,
    hidden: torch.Tensor,
    ...
    """Vectorized manual DP-SGD for Stage 3 using torch.func (much faster than per-sample loops).

    Computes per-sample gradients with vmap(value_and_grad), clips, adds noise, and steps.
    """
    if not TORCHFUNC_AVAILABLE:
        return _manual_dp_sgd_step(
            loss_fn=loss_fn,
            model=model,
            hidden=hidden,
            targets=targets,
    ...
    base = _unwrap_opacus_model(model)
    stage3_module = _Stage3HeadWrapper(
        regressor_nondelayed=base.regressor_nondelayed,
        stage3_time_embed=getattr(base, "stage3_time_embed", None),
    )

    # Only parameters (buffers are passed through functional_call but are not optimized here).
    params = dict(stage3_module.named_parameters())
    buffers = dict(stage3_module.named_buffers())
    if not params:
        return 0.0

    batch_size = int(hidden.shape[0])
    device = hidden.device
    microbatch_size = int(max(1, microbatch_size))

    # Accumulators in the same key order as params.
    summed_clipped: Dict[str, torch.Tensor] = {k: torch.zeros_like(v, device=device) for k, v in params.items()}

    effective_count = 0
    loss_accum = 0.0

    # Define per-sample loss for vmap.
    if time_features is None:
        def _per_sample_loss(p, b, h_i, y_i, m_i):
            pred = functional_call(stage3_module, (p, b), (h_i.unsqueeze(0), None))
            per_elem = loss_fn(pred, y_i.unsqueeze(0)) * m_i.unsqueeze(0)
            denom = m_i.sum().clamp_min(1.0)
            return (per_elem.sum() / denom)

        vg = value_and_grad(_per_sample_loss)

        def _chunk_vg(h_mb, y_mb, m_mb):
            return vmap(vg, in_dims=(None, None, 0, 0, 0))(params, buffers, h_mb, y_mb, m_mb)
    else:
        def _per_sample_loss(p, b, h_i, y_i, m_i, t_i):
            pred = functional_call(stage3_module, (p, b), (h_i.unsqueeze(0), t_i.unsqueeze(0)))
            per_elem = loss_fn(pred, y_i.unsqueeze(0)) * m_i.unsqueeze(0)
            denom = m_i.sum().clamp_min(1.0)
            return (per_elem.sum() / denom)
        ...
        def _chunk_vg(h_mb, y_mb, m_mb, t_mb):
            return vmap(vg, in_dims=(None, None, 0, 0, 0, 0))(params, buffers, h_mb, y_mb, m_mb, t_mb)

    for start in range(0, batch_size, microbatch_size):
        end = min(batch_size, start + microbatch_size)
        h_mb = hidden[start:end]
        y_mb = targets[start:end]
        m_mb = element_mask[start:end]

        # Count samples contributing any mask.
        keep = (m_mb.sum(dim=1) > 0)
        keep_count = int(keep.sum().item())
        effective_count += keep_count

        # Skip fully-masked samples to avoid wasting vmap compute.
        # This preserves DP scaling because masked samples contribute zero gradients.
        if keep_count <= 0:
            continue
        ...
        if time_features is None:
            losses_mb, grads_mb = _chunk_vg(h_mb, y_mb, m_mb)
        else:
            t_mb = time_features[start:end][keep]
            losses_mb, grads_mb = _chunk_vg(h_mb, y_mb, m_mb, t_mb)

        # losses_mb: [mb_kept]
        loss_accum += float(losses_mb.sum().item())

        # Compute per-sample L2 norms across params.
        mb = int(h_mb.shape[0])
        norms_sq = torch.zeros(mb, device=device)
        for g in grads_mb.values():
            norms_sq = norms_sq + g.reshape(mb, -1).pow(2).sum(dim=1)
        norms = torch.sqrt(norms_sq + 1e-12)
        clip_coef = (float(max_grad_norm) / (norms + 1e-12)).clamp(max=1.0)

        # Sum clipped gradients into accumulators.
        for name, g in grads_mb.items():
            view_shape = (mb,) + (1,) * (g.ndim - 1)
            summed_clipped[name] = summed_clipped[name] + (g * clip_coef.view(view_shape)).sum(dim=0)
    ...
    # Write .grad onto the real parameters (same objects referenced by stage3_module).
    for name, p in stage3_module.named_parameters():
        if not p.requires_grad:
            continue
        s = summed_clipped[name]
        p.grad = (s + torch.randn_like(s) * std) / float(batch_size)
...
def train_stage1_opacus(
    ...
    for p in base.regressor_delayed.parameters():
        p.requires_grad = False
    for p in base.regressor_nondelayed.parameters():
        p.requires_grad = False
...
def train_stage2_opacus(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_x, _, batch_y_reg in train_loader:
            ...
            optimizer.zero_grad(set_to_none=True)
            hidden = base._encode_x(batch_x)
            preds = base.forward_regressor(hidden, which="delayed")
        ...
        if val_loss < best_val_loss:
            ...
            best_state = copy.deepcopy(base.regressor_delayed.state_dict())
            print("  ✓ New best checkpoint")
    ...
    if best_state is not None:
        base.regressor_delayed.load_state_dict(best_state)
...
def train_stage3_opacus(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            if len(batch) == 4:
                batch_x, _, batch_y_reg, batch_time = batch
            else:
                batch_x, _, batch_y_reg = batch
                batch_time = None
            ...
            optimizer.zero_grad(set_to_none=True)
            hidden = base._encode_x(batch_x)
            preds = base.forward_regressor(hidden, which="nondelayed", time_features=batch_time)
        ...
        if val_loss < best_val_loss and val_nondelayed > 0:
            best_val_loss = val_loss
            best_state = copy.deepcopy(base.regressor_nondelayed.state_dict())
    ...
    if best_state is not None:
        base.regressor_nondelayed.load_state_dict(best_state)
...
def train_stage3_manual_dp(
    ...
    *,
    ...
    noise_multiplier: float,
    max_grad_norm: float,
    stage3_microbatch: int = 16,
    stage3_dp_impl: str = "vmap",
    stage3_epsilon_guard: str = "every_step",
    stage3_epsilon_guard_interval: int = 10,
    stage3_epsilon_guard_margin: float = 0.25,
) -> Tuple[List[Dict], float]:
    """Stage 3 with manual DP-SGD (for KAN/time head), accounting via Opacus accountant.

    This avoids Opacus' model wrapping limitations for custom modules with buffers.
    Epsilon is prevented from crossing target_epsilon by simulating the next step.
    """
    stage_start_time = time.time()
    print("\n" + "=" * 80)
    print("STAGE 3: TRAINING DELAY REGRESSOR (NON-DELAYED FLIGHTS) (MANUAL DP-SGD)")
    print(f"Training on flights with |delay| < {delay_threshold} min")
    print("=" * 80)

    if str(stage3_dp_impl).lower().strip() == "vmap" and not TORCHFUNC_AVAILABLE:
        print(
            "[STAGE 3 WARNING] --stage3_dp_impl vmap requested but torch.func is not available. "
            "Falling back to a slow per-sample Python loop. Install/upgrade to PyTorch 2.x "
            "(torch>=2.0) for fast vmap-based per-sample gradients.",
            flush=True,
        )
    ...
    for p in base.regressor_nondelayed.parameters():
        p.requires_grad = True
    if getattr(base, "stage3_time_embed", None) is not None:
        for p in base.stage3_time_embed.parameters():
            p.requires_grad = True

    stage3_params: List[torch.nn.Parameter] = list(base.regressor_nondelayed.parameters())
    if getattr(base, "stage3_time_embed", None) is not None:
        stage3_params += list(base.stage3_time_embed.parameters())
    stage3_params = [p for p in stage3_params if p.requires_grad]
    if not stage3_params:
        raise RuntimeError("Stage 3 manual DP selected but no trainable Stage 3 parameters were found")

    optimizer = torch.optim.Adam(stage3_params, lr=float(lr), weight_decay=1e-5)
    reg_loss_fn = nn.HuberLoss(reduction="none", delta=1.0)
    ...
    accountant = _get_accountant(privacy_engine)
    if accountant is None:
        # Fallback: local RDP accountant if PrivacyEngine wasn't provided.
        if RDPAccountant is None:
            raise RuntimeError("Opacus accountant not available for manual DP Stage 3")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            # Privacy budget guard: avoid crossing epsilon.
            # Note: _will_exceed_epsilon_next_step() deepcopies the accountant, which is expensive.
            # We allow running it every N steps unless close to the target.
            guard_mode = str(stage3_epsilon_guard).lower().strip()
            if accountant is not None and privacy_engine is not None and guard_mode != "none":
                do_check = (guard_mode == "every_step")
                if guard_mode == "interval":
                    interval = max(1, int(stage3_epsilon_guard_interval))
                    margin = max(0.0, float(stage3_epsilon_guard_margin))
                    do_check = (step_idx % interval == 0)
                    if last_eps_now == last_eps_now and last_eps_now >= (float(target_epsilon) - margin):
                        do_check = True

                if do_check:
                    would_exceed, eps_now, eps_next = _will_exceed_epsilon_next_step(
                        accountant,
                    ...
                    last_eps_now = eps_now
                    if eps_now == eps_now and eps_now >= float(target_epsilon):
                        stopped_for_budget = True
                        break
                    if would_exceed:
                        print(
                            f"[DP] Stopping before step would exceed ε: now={eps_now:.4f}, next={eps_next:.4f}, target={float(target_epsilon):.4f}",
                            flush=True,
                        )
                        stopped_for_budget = True
                        break
            ...
            use_vmap = (str(stage3_dp_impl).lower().strip() == "vmap")
            if use_vmap:
                loss_avg = _manual_dp_sgd_step_vmap(
                    loss_fn=reg_loss_fn,
                    model=base,
                    hidden=hidden,
                    targets=batch_y_reg,
                    ...
                    noise_multiplier=float(noise_multiplier),
                    time_features=batch_time,
                    microbatch_size=int(stage3_microbatch),
                )
            else:
                loss_avg = _manual_dp_sgd_step(
                    loss_fn=reg_loss_fn,
                    model=base,
                    hidden=hidden,
                    targets=batch_y_reg,
            ...
            if loss_avg > 0:
                epoch_losses.append(float(loss_avg))

            # Account for this DP step.
            if accountant is not None:
                try:
                    accountant.step(noise_multiplier=float(noise_multiplier), sample_rate=float(sample_rate))
                except Exception as e:
                    print(f"[DP WARNING] Failed accountant.step ({type(e).__name__}: {e})")
        ...
        epoch_time = time.time() - epoch_start_time
        current_epsilon = (
            float(accountant.get_epsilon(float(target_delta)))
            if accountant is not None
            else _safe_get_epsilon(privacy_engine, target_delta)
        )
        ...
        history.append(
            {
                ...
                "val_nondelayed_ratio": val_nondelayed_ratio,
                "epsilon": current_epsilon,
                "delta": target_delta if accountant is not None else 0.0,
                "epoch_time_seconds": epoch_time,
                "manual_dp": True,
                "noise_multiplier": float(noise_multiplier),
            }
        ...
        print(
            f"Epoch {epoch}/{epochs} | Loss: {history[-1]['train_loss']:.4f} | Val: {val_loss:.4f} | "
            f"Non-delayed: {total_nondelayed:.0f} ({nondelayed_ratio*100:.1f}%) | "
            f"Val ND: {val_nondelayed:.0f} ({val_nondelayed_ratio*100:.1f}%) | "
            f"ε: {current_epsilon:.3f}/{float(target_epsilon):.3f} | Time: {epoch_time:.2f}s",
            flush=True,
        )
        ...
        if val_loss < best_val_loss and val_nondelayed > 0:
            best_val_loss = val_loss
            best_state = copy.deepcopy(base.regressor_nondelayed.state_dict())
            print("  ✓ New best (Stage 3 manual DP)")
    ...
    print(f"\nStage 3 completed in {stage_time:.2f}s ({stage_time/60:.2f} min)")
    print(f"Final ε: {final_eps:.3f} (target: {float(target_epsilon):.3f})")
    if final_eps > float(target_epsilon) + 1e-6:
        print("[DP WARNING] ε exceeded target; consider lowering stage3_epochs or increasing noise.")
    return history, stage_time
...
def final_evaluation(
    ...
    num_nodes: int,
    test_x: torch.Tensor,
    test_time_x: Optional[torch.Tensor],
    ...
    *,
    ...
    if save_model:
        ...
        to_save = {
            ...
            'regressor': model.regressor_delayed.state_dict(),
    ...
    if isinstance(checkpoint, dict) and 'encoder' in checkpoint and 'classifier' in checkpoint:
        ...
        # Load delayed/non-delayed regressors (do not clone modules at load time).
        if 'regressor_delayed' in checkpoint and 'regressor_nondelayed' in checkpoint:
            ...
            # Legacy single-regressor checkpoint: initialize both regressors from the same weights.
    ...
    with torch.no_grad():
        for i in range(len(test_x)):
            time_i = test_time_x[i].to(device) if test_time_x is not None else None
            data = Data(
            ...
            if hasattr(model, 'regressor_delayed') and hasattr(model, 'regressor_nondelayed'):
                ...
                reg_nondelayed = model.forward_regressor(hidden, which="nondelayed", time_features=time_i)
...
def _resolve_checkpoint_path(maybe_path: Optional[str], default_filename: str, *, checkpoint_dir: str) -> str:
    if maybe_path is None or str(maybe_path).strip() == "":
        return os.path.join(checkpoint_dir, default_filename)

    p = str(maybe_path)
    # Absolute or explicitly relative path that exists.
    if os.path.isabs(p) and os.path.exists(p):
        return p
    if os.path.exists(p):
        return p
    # Otherwise interpret as relative to checkpoint_dir.
    return os.path.join(checkpoint_dir, p)


def _strip_known_prefixes_from_state_dict(state_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    prefixes = ("_module.", "module.")
    if not any(any(k.startswith(pref) for pref in prefixes) for k in state_dict.keys()):
        return state_dict
...
def _load_stage_checkpoint_into_model(
    model: nn.Module,
    checkpoint_path: str,
    *,
    load_encoder: bool = False,
    load_classifier: bool = False,
    load_regressor_delayed: bool = False,
    load_regressor_nondelayed: bool = False,
    load_stage3_time_embed: bool = False,
) -> None:
    ...
    # Preferred format: submodule state dicts.
    if isinstance(ckpt, dict) and ("encoder" in ckpt or "classifier" in ckpt or "regressor" in ckpt or "regressor_delayed" in ckpt):
        if load_encoder and "encoder" in ckpt:
            base.encoder.load_state_dict(ckpt["encoder"], strict=True)
        if load_classifier and "classifier" in ckpt:
            base.classifier.load_state_dict(ckpt["classifier"], strict=True)

        # Back-compat: treat 'regressor' as delayed regressor.
        if load_regressor_delayed:
            if "regressor_delayed" in ckpt:
                base.regressor_delayed.load_state_dict(ckpt["regressor_delayed"], strict=True)
            elif "regressor" in ckpt:
                base.regressor_delayed.load_state_dict(ckpt["regressor"], strict=True)

        if load_regressor_nondelayed and "regressor_nondelayed" in ckpt:
            base.regressor_nondelayed.load_state_dict(ckpt["regressor_nondelayed"], strict=True)

        if load_stage3_time_embed and getattr(base, "stage3_time_embed", None) is not None:
            if "stage3_time_embed" in ckpt:
                base.stage3_time_embed.load_state_dict(ckpt["stage3_time_embed"], strict=True)
    ...
    if isinstance(ckpt, dict) and ("state_dict" in ckpt or "model_state_dict" in ckpt):
        ...
        if not isinstance(sd, dict):
            raise ValueError("Checkpoint has 'state_dict' but it is not a dict")
        sd = _strip_known_prefixes_from_state_dict(sd)
        model.load_state_dict(sd, strict=False)
        return

    # Plain state_dict format
    if isinstance(ckpt, dict) and ckpt and all(isinstance(k, str) for k in ckpt.keys()):
        try:
            sd2 = _strip_known_prefixes_from_state_dict(ckpt)  # type: ignore[arg-type]
            model.load_state_dict(sd2, strict=False)
            return
        except Exception as e:
            raise ValueError(f"Unrecognized state_dict format in {checkpoint_path}: {e}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default='kan',
        choices=['mlp', 'kan'],
        help='Regressor head for Stage 3 (non-delayed). Use kan to match KAN-based head used in your other models.'
    )
    parser.add_argument('--kan_grid_size', type=int, default=3, help='KAN grid size (if --stage3_regressor kan)')
    parser.add_argument('--kan_spline_order', type=int, default=2, help='KAN spline order (if --stage3_regressor kan)')
    parser.add_argument(
        '--stage3_dp_impl',
        type=str,
        default='vmap',
        choices=['vmap', 'loop'],
        help=(
            "Stage 3 manual DP implementation. 'vmap' uses torch.func for fast per-sample grads; "
            "'loop' uses a slow Python per-sample loop (debug only)."
        ),
    )
    parser.add_argument(
        '--stage3_microbatch',
        ...
        default=16,
        help=(
            'Microbatch size used by Stage 3 manual DP when --stage3_dp_impl vmap. '
            'Lower reduces memory; higher is faster (GPU permitting).'
        ),
    )
    parser.add_argument(
        '--stage3_epsilon_guard',
        type=str,
        default='every_step',
        choices=['every_step', 'interval', 'none'],
        help=(
            "How to enforce the Stage 3 epsilon budget when using manual DP. "
            "'every_step' is safest but slowest (deepcopies accountant each step); "
            "'interval' checks every N steps and switches to every-step near the target; "
            "'none' disables the pre-step guard (not recommended)."
        ),
    )
    parser.add_argument(
        '--stage3_epsilon_guard_interval',
        type=int,
        default=10,
        help="In 'interval' mode, run the expensive epsilon-next-step check every N steps.",
    )
    parser.add_argument(
        '--stage3_epsilon_guard_margin',
        type=float,
        default=0.25,
        help="In 'interval' mode, switch to every-step checks once eps_now >= target - margin.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='DataLoader worker processes (0 disables multiprocessing; Windows default 0 is safest).',
    )
    parser.add_argument(
        '--pin_memory',
        dest='pin_memory',
        action='store_true',
        default=None,
        help='Enable DataLoader pin_memory (recommended on CUDA).',
    )
    parser.add_argument(
        '--no-pin_memory',
        dest='pin_memory',
    ...
    parser.add_argument('--lr', type=float, default=0.009, help='Global learning rate (used if stage-specific LRs not provided)')
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            'Skip Stage 1/2 training and run ONLY Stage 3. '
            'Stage 1/2 weights must be available via --stage1_checkpoint/--stage2_checkpoint '
            "(or as stage1_checkpoint.pth/stage2_checkpoint.pth inside --checkpoint_dir)."
        ),
    )
    parser.add_argument(
        '--stage1_checkpoint',
        type=str,
        default=None,
        help='Path to Stage 1 checkpoint (encoder+classifier). If omitted, uses <checkpoint_dir>/stage1_checkpoint.pth.',
    )
    parser.add_argument(
        '--stage2_checkpoint',
        type=str,
        default=None,
        help='Path to Stage 2 checkpoint (delayed regressor). If omitted, uses <checkpoint_dir>/stage2_checkpoint.pth.',
    )
    parser.add_argument(
        '--stage3_checkpoint',
        type=str,
        default=None,
        help='Optional Stage 3 checkpoint to initialize Stage 3 head before continuing training.',
    )
    ...
    parser.add_argument('--no-dp', dest='dp', action='store_false', help='Disable DP-SGD (debug/speed).')
    parser.add_argument(
        '--epsilonfixed',
        dest='epsilonfixed',
        default=False,
...
def main() -> None:
    ...
    # Time features are assumed to be the last 2 columns: [hour, day_of_week]
    # We keep a separate copy for Stage 3, even if excluded from the encoder input.
    time_feature_dim = 0
    train_time_inputs = val_time_inputs = test_time_inputs = None
    if train_inputs.shape[2] >= 2:
        time_feature_dim = 2
        train_time_inputs = train_inputs[:, :, -2:].copy() if hasattr(train_inputs, "copy") else train_inputs[:, :, -2:].clone()
        val_time_inputs = val_inputs[:, :, -2:].copy() if hasattr(val_inputs, "copy") else val_inputs[:, :, -2:].clone()
        test_time_inputs = test_inputs[:, :, -2:].copy() if hasattr(test_inputs, "copy") else test_inputs[:, :, -2:].clone()

    # Optionally exclude time features from the encoder input
    if args.exclude_time_features:
        print(f"\n[FEATURE FILTERING] Excluding time features from encoder input")
        ...
        print(f"  New encoder feature dimension: {train_inputs.shape[2]}")
    
    feature_dim = train_inputs.shape[2]
    delay_dim = train_delay_scaled.shape[2]
    # Keep node features as [N, F, T] (do not flatten into T*F).
    ...
    # Ensure feature tensors are [S, N, F, T] so node-level batches become [B, F, T].
    train_x = _ensure_node_features_s_n_f_t(train_x, args.seq_len)
    val_x = _ensure_node_features_s_n_f_t(val_x, args.seq_len)
    test_x = _ensure_node_features_s_n_f_t(test_x, args.seq_len)

    # Build parallel sequences for time-only features (used only in Stage 3).
    train_time_x = val_time_x = test_time_x = None
    if train_time_inputs is not None and time_feature_dim > 0:
        train_time_x, _, _ = build_fn(
            train_time_inputs,
            train_delay_scaled,
            train_raw,
            args.seq_len,
    ...
    if args.balance_50_50:
        ...
        if n_pos > 0 and n_neg > 0:
            ...
            if train_time_x is not None:
                train_time_x = train_time_x[combined_indices]
    ...
    model = SequentialTwoStagePredictor(
        ...
        stage3_regressor=str(args.stage3_regressor),
        kan_grid_size=int(args.kan_grid_size),
        kan_spline_order=int(args.kan_spline_order),
        stage3_use_time_features=True if time_feature_dim > 0 else False,
        stage3_time_feature_dim=int(time_feature_dim) if time_feature_dim > 0 else 0,
    ).to(device)

    # If requested, load Stage 1/2 checkpoints and run only Stage 3.
    if bool(getattr(args, "stage3_only", False)):
        stage1_ckpt = _resolve_checkpoint_path(getattr(args, "stage1_checkpoint", None), "stage1_checkpoint.pth", checkpoint_dir=CHECKPOINT_DIR)
        stage2_ckpt = _resolve_checkpoint_path(getattr(args, "stage2_checkpoint", None), "stage2_checkpoint.pth", checkpoint_dir=CHECKPOINT_DIR)
        stage3_ckpt = None
        if getattr(args, "stage3_checkpoint", None):
            stage3_ckpt = _resolve_checkpoint_path(getattr(args, "stage3_checkpoint", None), "stage3_checkpoint.pth", checkpoint_dir=CHECKPOINT_DIR)

        print("\n" + "=" * 80)
        print("RESUME MODE: STAGE 3 ONLY")
        print("=" * 80)
        print(f"Loading Stage 1 checkpoint: {stage1_ckpt}")
        _load_stage_checkpoint_into_model(model, stage1_ckpt, load_encoder=True, load_classifier=True)
        print(f"Loading Stage 2 checkpoint: {stage2_ckpt}")
        _load_stage_checkpoint_into_model(model, stage2_ckpt, load_regressor_delayed=True)
        if stage3_ckpt is not None and os.path.exists(stage3_ckpt):
            print(f"Loading Stage 3 checkpoint (optional init): {stage3_ckpt}")
            _load_stage_checkpoint_into_model(
                model,
                stage3_ckpt,
                load_regressor_nondelayed=True,
                load_stage3_time_embed=True,
            )
        print("✓ Loaded checkpoints. Stage 1/2 training will be skipped; running only Stage 3.")
    ...
    loader_num_workers = max(0, int(getattr(args, "num_workers", 0)))
    if getattr(args, "pin_memory", None) is None:
        loader_pin_memory = (device.type == "cuda")
    else:
        loader_pin_memory = bool(getattr(args, "pin_memory"))
    ...
    if loader_num_workers > 0:
        dl_params = set(inspect.signature(DataLoader.__init__).parameters)
        if "persistent_workers" in dl_params:
            common_loader_kwargs["persistent_workers"] = True
        if "prefetch_factor" in dl_params:
            common_loader_kwargs["prefetch_factor"] = max(1, int(getattr(args, "prefetch_factor", 2)))

    # Stage 3 loader can optionally include time features.
    train_loader_stage3 = None
    val_loader_stage3 = None
    if train_time_x is not None:
        train_time_flat = _flatten_node_level(train_time_x)
        val_time_flat = _flatten_node_level(val_time_x)
        train_ds_stage3 = TensorDataset(train_x_flat, train_y_cls_flat, train_y_reg_flat, train_time_flat)
        val_ds_stage3 = TensorDataset(val_x_flat, val_y_cls_flat, val_y_reg_flat, val_time_flat)
        train_loader_stage3 = DataLoader(train_ds_stage3, shuffle=True, drop_last=True, **common_loader_kwargs)
        val_loader_stage3 = DataLoader(val_ds_stage3, shuffle=False, drop_last=False, **common_loader_kwargs)
    else:
        train_loader_stage3 = DataLoader(train_ds, shuffle=True, drop_last=True, **common_loader_kwargs)
        val_loader_stage3 = DataLoader(val_ds, shuffle=False, drop_last=False, **common_loader_kwargs)
    ...
    # If running only Stage 3, calibrate DP for Stage 3 epochs only.
    stage1_epochs_to_run = int(args.stage1_epochs) if not bool(getattr(args, "stage3_only", False)) else 0
    stage2_epochs_to_run = int(args.stage2_epochs) if not bool(getattr(args, "stage3_only", False)) else 0
    stage3_epochs_to_run = int(args.stage3_epochs)
    total_epochs = stage1_epochs_to_run + stage2_epochs_to_run + stage3_epochs_to_run
    ...
    train_loader = DataLoader(train_ds, shuffle=True, drop_last=True, **common_loader_kwargs)
    val_loader = DataLoader(val_ds, shuffle=False, drop_last=False, **common_loader_kwargs)
    ...
    if dp_enabled:
        ...
        # If Stage 3 uses a custom KAN module, Opacus' hook-based per-sample grads
        # may not support it depending on version. Prefer functorch grad-sampling.
        grad_sample_mode = "functorch" if str(args.stage3_regressor).lower() == "kan" else "hooks"
        if str(args.stage3_regressor).lower() == "kan":
            print("[DP] Stage 3 uses KAN: using grad_sample_mode=functorch for Opacus compatibility")

        # IMPORTANT: If Stage 3 uses KAN (and/or the Stage 3 time embed), Opacus may not
        # support it when trainable. We freeze Stage-3-only modules for Opacus wrapping,
        # then train them later with manual DP-SGD in Stage 3.
        stage3_manual_dp = str(args.stage3_regressor).lower().strip() == "kan"
        if stage3_manual_dp:
            print("[DP] Freezing Stage 3 head for Opacus init; Stage 3 will use manual DP-SGD")
            for p in model.regressor_nondelayed.parameters():
                p.requires_grad = False
            if getattr(model, "stage3_time_embed", None) is not None:
                for p in model.stage3_time_embed.parameters():
                    p.requires_grad = False
        ...
        if ModuleValidator is not None:
            ...
            try:
                ...
                print(f"[DP WARNING] ModuleValidator strict validation failed ({type(e).__name__}: {e}). Trying strict=False.")
        ...
        if getattr(args, "epsilonfixed", False):
            ...
            grad_sample_mode_applied = False
            try:
                ...
                grad_sample_mode_applied = True
            except TypeError:
                # Older Opacus versions may not support grad_sample_mode.
                model, optimizer, train_loader = privacy_engine.make_private_with_epsilon(
                    module=model,
                    optimizer=optimizer,
                    data_loader=train_loader,
            ...
            if str(args.stage3_regressor).lower() == "kan" and not grad_sample_mode_applied:
                raise RuntimeError(
                    "Stage 3 regressor is set to KAN under DP, but your Opacus version does not "
                    "support grad_sample_mode='functorch'. Upgrade Opacus (pip install -U opacus) "
                    "or run with --stage3_regressor mlp."
                )
    ...
    if stage1_epochs_to_run > 0:
        ...
        history_s1, stage1_time = [], 0.0
        print("\n[STAGE 1] Skipped (stage3_only)")
    ...
    if stage2_epochs_to_run > 0:
        ...
        history_s2, stage2_time = [], 0.0
        print("\n[STAGE 2] Skipped (stage3_only)")

    # Dual regressors are permanent modules; Stage 2 already trained regressor_delayed.

    if dp_enabled and stage3_manual_dp:
        history_s3, stage3_time = train_stage3_manual_dp(
            model=model,
            privacy_engine=privacy_engine,
            train_loader=train_loader_stage3,
            val_loader=val_loader_stage3,
            device=device,
            epochs=stage3_epochs_to_run,
            lr=stage3_lr,
            scaler=scaler,
            delay_threshold=float(args.delay_threshold),
            patience=args.patience,
            ...
            max_grad_norm=float(args.max_grad_norm),
            stage3_microbatch=int(args.stage3_microbatch),
            stage3_dp_impl=str(args.stage3_dp_impl),
            stage3_epsilon_guard=str(getattr(args, "stage3_epsilon_guard", "every_step")),
            stage3_epsilon_guard_interval=int(getattr(args, "stage3_epsilon_guard_interval", 10)),
            stage3_epsilon_guard_margin=float(getattr(args, "stage3_epsilon_guard_margin", 0.25)),
        )
    else:
    ...
    # Dual regressors are permanent modules; Stage 3 already trained regressor_nondelayed.
