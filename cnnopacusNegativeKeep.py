...
def train_stage3_opacus(
    ...
    This includes negative delays (early arrivals).
    ...
    print(f"Training on flights with delay <= {delay_threshold} min (includes negative early arrivals)")
    print("=" * 80)
    ...
    def nondelayed_mask_from_targets(targets_scaled: torch.Tensor) -> torch.Tensor:
        ...
        # Non-delayed set includes all early arrivals (negative) and on-time flights.
        return (targets_denorm <= float(delay_threshold)).float()
...
def final_evaluation(
    ...
    *,
    ...
    # Regression should be evaluated over the full delay range (including negatives).
    # Do not gate non-delayed samples to 0.
    ...
    # Keep negative delays as-is (early arrivals).
...
def main() -> None:
    ...
    def _report_negative_delays(tag: str, raw_arr: np.ndarray) -> None:
        if raw_arr.size == 0:
            print(f"  [{tag}] raw delay: empty")
            return
        raw_2d = raw_arr.reshape(-1, raw_arr.shape[-1]) if raw_arr.ndim >= 2 else raw_arr.reshape(-1, 1)
        mins = np.nanmin(raw_2d, axis=0)
        maxs = np.nanmax(raw_2d, axis=0)
        neg_counts = np.sum(raw_2d < 0, axis=0)
        totals = raw_2d.shape[0]
        neg_fracs = neg_counts / max(1, totals)
        if raw_2d.shape[1] >= 2:
            print(
                f"  [{tag}] raw delays: min(arr/dep)={mins[0]:.2f}/{mins[1]:.2f}, "
                f"max(arr/dep)={maxs[0]:.2f}/{maxs[1]:.2f}, "
                f"neg%(arr/dep)={neg_fracs[0]*100:.2f}%/{neg_fracs[1]*100:.2f}%"
            )
        else:
            print(
                f"  [{tag}] raw delays: min={mins[0]:.2f}, max={maxs[0]:.2f}, neg%={neg_fracs[0]*100:.2f}%"
            )

    print("\n[RAW DELAY CHECK] Verifying negative delays are preserved...")
    _report_negative_delays("train", train_raw)
    _report_negative_delays("val", val_raw)
    _report_negative_delays("test", test_raw)
    ...
    # Confirm regression targets still include negative values (in scaled space).
    # Note: scaled negatives do not necessarily mean early arrival, but this helps catch accidental clipping.
    if isinstance(train_y_reg, torch.Tensor):
        neg_scaled = (train_y_reg < 0).float().mean().item()
        print(f"\n[TARGET CHECK] Fraction of scaled regression targets < 0: {neg_scaled*100:.2f}%")
