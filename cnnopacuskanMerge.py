...
from classifykat import (  # noqa: E402
    EarlyStopping,
    SequentialTwoStagePredictor as KANGATPredictor,  # KAN-GAT model for Stage 3
    LightweightGATEncoder,  # KAN-GAT encoder for Stage 3
    build_sequences,
...
class HybridCNNKANGATPredictor(nn.Module):
    """Hybrid model: CNN encoder for Stages 1 & 2, KAN-GAT encoder for Stage 3.
    
    This architecture uses:
    - CNN-based encoder + heads for Stage 1 (classification) and Stage 2 (delayed regression)
    - KAN-GAT encoder + regressor for Stage 3 (non-delayed regression)
    
    The KAN-GAT model operates on graph-structured data with edge indices, while
    the CNN model operates on flattened sequences.
    """
    ...
    def __init__(
        ...
        # CNN model for Stages 1 & 2
        self.cnn_model = SequentialTwoStagePredictor(
            in_channels=in_channels,
            out_channels=out_channels,
            hidden_channels=hidden_channels,
            regressor_extra_layer=regressor_extra_layer,
            seq_len=seq_len,
        )
        
        # KAN-GAT model for Stage 3 (non-delayed regression)
        self.kangat_model = KANGATPredictor(
            in_channels=in_channels,
            out_channels=out_channels,
            hidden_channels=hidden_channels,
            regressor_extra_layer=regressor_extra_layer,
        ...
        # Aliases to support Opacus model validation and standard training loops
        self.encoder = self.cnn_model.encoder
        self.classifier = self.cnn_model.classifier
        self.regressor = self.cnn_model.regressor  # Stage 2 delayed regressor (CNN)
        
        # Flag to track which stage we're training
        self._stage = 1
        
        print(f"[HybridCNNKANGATPredictor] Initialized:")
        print(f"  - CNN model for Stages 1 & 2 (classification + delayed regression)")
        print(f"  - KAN-GAT model for Stage 3 (non-delayed regression)")

    def set_stage(self, stage: int) -> None:
        """Set the current training stage (1, 2, or 3)."""
        self._stage = stage
        print(f"[HybridCNNKANGATPredictor] Switched to Stage {stage}")

    def _encode_x(self, x: torch.Tensor) -> torch.Tensor:
        """CNN encoding for node-level flat features."""
        return self.cnn_model._encode_x(x)

    def forward_classifier(self, data: Data) -> Tuple[torch.Tensor, torch.Tensor]:
        """Stage 1: Classification using CNN encoder."""
        return self.cnn_model.forward_classifier(data)

    def forward_regressor(self, hidden: torch.Tensor) -> torch.Tensor:
        """Stage 2: Regression using CNN regressor on CNN-encoded hidden."""
        return self.cnn_model.forward_regressor(hidden)

    def forward_kangat_regressor(self, data: Data) -> Tuple[torch.Tensor, torch.Tensor]:
        """Stage 3: Non-delayed regression using KAN-GAT model.
        
        Returns (hidden, regression_output) from KAN-GAT.
        """
        return self.kangat_model(data)

    def forward(self, data: Data) -> Tuple[torch.Tensor, torch.Tensor]:
        """Full forward pass - uses CNN model for default."""
        return self.cnn_model.forward(data)
...
def train_stage3_kangat(
    kangat_model: nn.Module,
    ...
    """Stage 3: Train KAN-GAT model for non-delayed flight regression.
    
    This function trains a separate KAN-GAT model (from classifykat.py) for
    predicting delays on non-delayed flights (|delay| < threshold).
    
    Unlike the CNN-based Stage 1/2, this uses graph-structured data with
    edge indices for the GAT attention mechanism.
    """
    stage_start_time = time.time()
    print("\n" + "=" * 80)
    print("STAGE 3: TRAINING KAN-GAT REGRESSOR (NON-DELAYED FLIGHTS)")
    print(f"Training on flights with |delay| < {delay_threshold} min")
    print("=" * 80)
    print(f"Train samples: {len(train_x)} | Val samples: {len(val_x)}")
    
    # Freeze encoder and classifier, train only the regressor
    for param in kangat_model.encoder.parameters():
        param.requires_grad = False
    for param in kangat_model.classifier.parameters():
        param.requires_grad = False
    for param in kangat_model.regressor.parameters():
        param.requires_grad = True
    
    # Lower learning rate for fine-tuning regressor
    optimizer = torch.optim.Adam(
        kangat_model.regressor.parameters(),
        lr=lr * 0.01,
        weight_decay=1e-5,
    )
    ...
    print(f"✓ KAN-GAT regressor-only training with LR={lr * 0.01:.6f}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for start_idx in range(0, num_samples, batch_size):
            ...
            for i in range(len(batch_x)):
                # Create PyG Data object with edge indices for KAN-GAT
                data = Data(
                    x=batch_x[i],
                ...
                # KAN-GAT forward returns (logits, regression_output)
                _, node_reg = kangat_model(data.to(device))
            ...
            # Create element-wise mask for non-delayed values
            ...
            if num_nondelayed > 0:
                # Huber loss in NORMALIZED space, masked by denorm threshold
        ...
        nondelayed_ratio = total_nondelayed / total_values if total_values > 0 else 0.0
        ...
        with torch.no_grad():
            for start_idx in range(0, len(val_x), batch_size):
                ...
                batch_x = val_x[start_idx:end_idx].to(device)
                batch_y_reg = val_y_reg[start_idx:end_idx].to(device)
                ...
                if scaler is not None:
                    targets_denorm = torch.from_numpy(
                        scaler.inverse_transform(reg_targets_t.cpu().numpy())
                    ).to(device)
                    preds_denorm = torch.from_numpy(
                        scaler.inverse_transform(reg_preds_t.cpu().numpy())
                    ).to(device)
                else:
                    targets_denorm = reg_targets_t
                    preds_denorm = reg_preds_t
                ...
                if num_nondelayed > 0:
                    se = ((preds_denorm - targets_denorm) ** 2) * element_mask
                    denom = element_mask.sum(dim=0).clamp_min(1.0)
                    loss_val_ch = se.sum(dim=0) / denom
                    val_losses.append(float(loss_val_ch.mean().item()))
        
        val_loss = float(np.mean(val_losses)) if val_losses else 0.0
        val_nondelayed_ratio = val_nondelayed / val_total if val_total > 0 else 0.0
        ...
        print(
            f"Epoch {epoch}/{epochs} | Loss: {history[-1]['train_loss']:.4f} | "
            f"Val MSE: {val_loss:.4f} | Non-delayed: {total_nondelayed:.0f} "
            f"({nondelayed_ratio*100:.1f}%) | Val ND: {val_nondelayed:.0f} "
            f"({val_nondelayed_ratio*100:.1f}%) | Time: {epoch_time:.2f}s",
            flush=True
        )
        
        if val_loss < best_val_loss and val_nondelayed > 0:
            best_val_loss = val_loss
            best_state = copy.deepcopy(kangat_model.regressor.state_dict())
            print("  ✓ New best (Stage 3 KAN-GAT)")
    ...
    if best_state is not None:
        kangat_model.regressor.load_state_dict(best_state)
    
    # Unfreeze all parameters
    for p in kangat_model.parameters():
        p.requires_grad = True
    
    stage_time = time.time() - stage_start_time
    print(f"\nStage 3 (KAN-GAT) completed in {stage_time:.2f}s ({stage_time/60:.2f} min)")
    ...
    *,
    ...
    if save_model:
        ...
        to_save = {
            ...
            'architecture': 'hybrid_cnn_kangat',  # Flag for hybrid architecture
        }
        if hasattr(model, 'regressor_delayed'):
            to_save['regressor_delayed'] = model.regressor_delayed.state_dict()
        # Save KAN-GAT model components for Stage 3 non-delayed regression
        if hasattr(model, 'kangat_model'):
            to_save['kangat_encoder'] = model.kangat_model.encoder.state_dict()
            to_save['kangat_regressor'] = model.kangat_model.regressor.state_dict()
            to_save['kangat_classifier'] = model.kangat_model.classifier.state_dict()
            to_save['use_kangat_stage3'] = True
        elif hasattr(model, 'regressor_nondelayed'):
    ...
    # Check if we have a KAN-GAT model for Stage 3 non-delayed regression
    kangat_model = getattr(model, 'kangat_model', None)
    use_kangat_stage3 = kangat_model is not None
    if use_kangat_stage3:
        kangat_model.eval()
        print("[EVALUATION] Using hybrid architecture: CNN (Stage 1/2) + KAN-GAT (Stage 3)")
    ...
    with torch.no_grad():
        for i in range(len(test_x)):
            ...
            # Always compute classifier once using CNN encoder.
            hidden, node_logits = model.forward_classifier(data)
            probs = torch.sigmoid(node_logits)

            # Hybrid routing: CNN delayed regressor + KAN-GAT non-delayed regressor
            if use_kangat_stage3 and hasattr(model, 'regressor_delayed'):
                # CNN delayed regressor on CNN embeddings
                hidden_dropped = model.dropout_reg(hidden)
                reg_delayed = model.regressor_delayed(hidden_dropped)
                
                # KAN-GAT non-delayed regressor (uses its own encoder)
                _, reg_nondelayed = kangat_model(data)
            ...
            # Fallback: both regressors from CNN (legacy)
            elif hasattr(model, 'regressor_delayed') and hasattr(model, 'regressor_nondelayed'):
                hidden_dropped = model.dropout_reg(hidden)
                reg_delayed = model.regressor_delayed(hidden_dropped)
    ...
    # NOTE: If using hybrid model (KAN-GAT Stage 3) or dual-regressor checkpoint,
    # routing was already applied in the per-sample loop above.
    # For legacy single-regressor checkpoints, apply delayed-only gating.
    if use_kangat_stage3 or ('regressor_delayed' in checkpoint and ('kangat_regressor' in checkpoint or 'regressor_nondelayed' in checkpoint)):
...
def main() -> None:
    ...
    # Stage 3: Create and train KAN-GAT model for non-delayed flights
    print("\n" + "=" * 80)
    print("INITIALIZING KAN-GAT MODEL FOR STAGE 3 (NON-DELAYED REGRESSION)")
    print("=" * 80)
    
    kangat_model = KANGATPredictor(
        in_channels=in_channels,
        out_channels=out_channels,
        hidden_channels=args.hidden_channels,
        regressor_extra_layer=True,
    ...
    print(f"[KAN-GAT Model] Created for Stage 3 non-delayed regression")
    print(f"  - in_channels: {in_channels}")
    print(f"  - out_channels: {out_channels}")
    print(f"  - hidden_channels: {args.hidden_channels}")
    
    history_s3, stage3_time = train_stage3_kangat(
        kangat_model=kangat_model,
        train_x=train_x,
    ...
    # Store the KAN-GAT regressor as the non-delayed regressor on the CNN model
    base_model.regressor_nondelayed_kangat = kangat_model.regressor
    base_model.kangat_encoder = kangat_model.encoder
    setattr(model, "regressor_nondelayed_kangat", kangat_model.regressor)
    setattr(model, "kangat_encoder", kangat_model.encoder)
    setattr(model, "kangat_model", kangat_model)  # Store full model for evaluation
