...
parser.add_argument('--max_grad_norm', type=float, default=2, help='per-sample gradient clipping norm')
args = parser.parse_args()

class TrainWindowDataset(Dataset):
    def _init_(self, data, weather, period, in_len, out_len, indices):
        self.data = data
...
def main():
    ...
    print(f"Normalized data range: [{training_data.min():.4f}, {training_data.max():.4f}]")
    print(f"Normalized data mean: {training_data.mean():.4f}, std: {training_data.std():.4f}")
    ...
    for ep in range(1, 1 + args.episode):
        ...
        with torch.no_grad():
            for i in range(len(val_index)):
                ...
                # Check for invalid values before inverse transform
                if np.isnan(out).any() or np.isinf(out).any():
                    print(f"WARNING: Invalid values in model output for val sample {i}")
                ...
                if np.isnan(out).any() or np.isinf(out).any():
                    print(f"WARNING: Invalid values after inverse transform for val sample {i}")
                    out = np.nan_to_num(out, nan=0.0, posinf=0.0, neginf=0.0)
