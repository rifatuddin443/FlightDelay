...
class linear(nn.Module):
    def __init__(self,c_in,c_out):
        ...
        # self.weight = nn.Parameter(torch.randn(c_out, c_in))
        # self.bias = nn.Parameter(torch.randn(c_out))

    def forward(self,x):
        # # x shape: (batch, c_in, height, width)
        # batch, c_in, h, w = x.shape
        # # Reshape to (batch, c_in, height*width)
        # x = x.view(batch, c_in, h*w)
        # # Manual linear transformation: weight @ x + bias
        # x = torch.einsum('oi,bik->bok', self.weight, x) + self.bias.view(-1, 1)
        # # Reshape back to (batch, c_out, height, width)
        # x = x.view(batch, -1, h, w)
        # return x
        #return self.mlp(x)

        # Input: (batch, in_features, height, width)
        batch_size, in_features, height, width = x.shape
        
        # Reshape to (batch * height * width, in_features)
        x_flat = x.permute(0, 2, 3, 1).contiguous().view(-1, in_features)
        
        # Apply linear transformation
        out_flat = self.linear(x_flat)  # (batch * height * width, out_features)
        
        # Reshape back to (batch, out_features, height, width)
        out = out_flat.view(batch_size, height, width, -1).permute(0, 3, 1, 2).contiguous()
...
class learnEmbedding(nn.Module):
    def __init__(self, d_model):
        ...
        self.factor = nn.parameter.Parameter(torch.randn(1,),requires_grad=True).to('cpu')
        self.d_model = d_model#d=frequency
    
    def forward(self, x):
        div = torch.arange(0, self.d_model, 2).to('cpu') #Generates a tensor of indices [0, 2, 4, ...] up to d_model.
        div_term = torch.exp(div * self.factor)
        if len(x.shape) == 2:
            v1 = torch.sin(torch.einsum('bt, f->btf', x, div_term)) #x has two dimensions[b,t], div_term [f]
    ...
    #[sin(x*f1), sin(x*f2) (v1), cos(x*f1), cos(x*f2) (its v2)]
    #If v1 shape is [batch, time, f] and v2 shape is also [batch, time, f],
    # after concatenation along last dimension (-1), the output shape is [batch, time, 2*f].
    
class ATT(nn.Module):
    def __init__(self,c_in, d = 16, device = 'cpu'):
...
class STMH_GCNN_layer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, emb_size, dropout, time_d = 16, heads = 4, support_len = 1, order = 2, final_layer = False):
        ...
        self.prelu = nn.ReLU()#PReLU()
        self.final_layer = final_layer
        self.dropout = dropout
        self.heads = heads
        
    def forward(self, x, t_in, supports, t_out = None):
        t_att = [] #store temporal attention matrices computed by each attention head.
        ...
        # Handle residual connection for nn.Linear
        if not isinstance(self.residual, nn.Identity):
            # Reshape x for nn.Linear: (batch, in_channels, V, T) -> (batch, V, T, in_channels)
            batch, c_in, v, t = x.shape
            x_reshaped = x.permute(0, 2, 3, 1)  # (batch, V, T, in_channels)
            res = self.residual(x_reshaped)     # (batch, V, T, out_channels)
            res = res.permute(0, 3, 1, 2)       # (batch, out_channels, V, T)
        else:
            res = self.residual(x)
