...
class linear(nn.Module):
    def __init__(self,c_in,c_out):
        ...
        # Use Conv2d instead of Linear for better Opacus compatibility
        # Conv2d with 1x1 kernel is equivalent to linear transformation
        self.conv = torch.nn.Conv2d(c_in, c_out, kernel_size=1, padding=0, stride=1, bias=True)

    def forward(self,x):
        # Input: (batch, in_channels, height, width)
        # Conv2d can handle this directly without reshaping
        return self.conv(x)
...
class learnEmbedding(nn.Module):
    ...
    def forward(self, x):
        # Compute div tensor dynamically to avoid any buffer issues with Opacus
        device = x.device
        dtype = x.dtype
