"""Single-pass differentially private KAN-GAT regression (no stages).

This script trains one end-to-end regressor (encoder + regressor head) to predict
scaled delays, with optional DP-SGD accounting.
"""
    ...
    def compute_per_sample_gradients(
        ...
        for i in range(batch_x.shape[0]):
            ...
            if is_classification:
                ...
                # Regression-only path: do NOT mask out negative delays
                hidden = self.model.encoder(data)
                node_reg = self.model.forward_regressor(hidden)
...
def train_regression_with_dp(
    ...
    stage_start_time = time.time()
    print("\n" + "=" * 80)
    print("REGRESSION: TRAINING ENCODER + REGRESSOR" + (" WITH DP-SGD" if dp_config.enabled else ""))
    print("=" * 80)
    print(f"Train samples: {len(train_x)} | Val samples: {len(val_x)}")

    # Freeze classifier; we are not doing classification.
    for param in model.classifier.parameters():
        param.requires_grad = False

    trainable_params = list(model.encoder.parameters()) + list(model.regressor.parameters())
    optimizer = torch.optim.Adam(trainable_params, lr=lr, weight_decay=1e-4)
    ...
    # Weighted loss to reduce error on delayed flights (imbalance-aware).
    # We compute the delay threshold in scaled space so we can weight targets
    # without denormalizing every batch.
    if scaler is not None and getattr(scaler, "std", None) is not None:
        thr_scaled = (float(delay_threshold_minutes) - float(scaler.mean)) / float(scaler.std)
    else:
        thr_scaled = float(delay_threshold_minutes)
    ...
    # Checkpoint stores the *last completed* epoch.
    start_epoch = 0
    checkpoint_path = os.path.join(CHECKPOINT_DIR, "regression_checkpoint.pth") if CHECKPOINT_DIR else None
    if checkpoint_path and os.path.exists(checkpoint_path):
        start_epoch, _ = load_checkpoint(model, optimizer, checkpoint_path)
        print(f"Resuming regression training from epoch {start_epoch}")
    ...
    for epoch in range(start_epoch + 1, epochs + 1):
        ...
        for start_idx in range(0, num_samples, batch_size):
            ...
            batch_indices = indices[start_idx:end_idx]
            batch_x = train_x[batch_indices].to(device)
            batch_y = train_y_reg[batch_indices].to(device)

            optimizer.zero_grad(set_to_none=True)

            if dp_config.enabled:
                per_sample_grads, _ = clipper.compute_per_sample_gradients(
                    batch_x, batch_y, edge_indices, reg_loss_fn, is_classification=False
                )
                noisy_grads = clipper.add_noise_to_gradients(
                    per_sample_grads,
                    dp_config.noise_multiplier,
                ...
                with torch.no_grad():
                    ...
                    for i in range(len(batch_x)):
                        ...
                        hidden = model.encoder(data.to(device))
                        node_reg = model.forward_regressor(hidden)
                        preds_list.append(node_reg)
                        targets_list.append(batch_y[i])
                    all_preds = torch.cat(preds_list, dim=0)
                    all_targets = torch.cat(targets_list, dim=0)
                    loss = reg_loss_fn(all_preds, all_targets)
        ...
        with torch.no_grad():
            ...
            for i in range(len(val_x)):
                ...
                node_reg = model.forward_regressor(hidden)
                val_preds.append(node_reg.cpu())
                val_targets.append(val_y_reg[i].cpu())

            val_preds_t = torch.cat(val_preds, dim=0)
            val_targets_t = torch.cat(val_targets, dim=0)
            val_loss = reg_loss_fn(val_preds_t, val_targets_t).item()
        ...
        history.append(
            {
                ...
                "delta": float(dp_config.target_delta) if dp_config.enabled else 0.0,
                "epoch_time_seconds": float(epoch_time),
                "total_steps": int(accountant.steps),
            }
        )

        eps_str = f"ε: {current_epsilon:.3f}/{dp_config.target_epsilon}" if dp_config.enabled else "No DP"
        print(
            f"Epoch {epoch}/{epochs} | TrainLoss: {history[-1]['train_loss']:.4f} | "
            f"ValLoss: {history[-1]['val_loss']:.4f} | LR: {current_lr:.6f} | {eps_str} | Time: {epoch_time:.2f}s"
        )
...
def final_evaluation(
    ...
    """Final evaluation and export (legacy staged pipeline)."""
    print("\n" + "="*80)
...
def final_regression_evaluation(
    model: SequentialTwoStagePredictor,
    edge_indices: Tuple,
    device: torch.device,
    scaler,
    ...
    print("\n" + "=" * 80)
    print("FINAL TEST EVALUATION (REGRESSION)")
    print("=" * 80)
    print(f"Test samples: {len(test_x)}")
    ...
    torch.save(
        {
            ...
            "final_delta": float(final_delta),
            "target_epsilon": float(dp_config.target_epsilon),
            "epsilon_exceeded": bool(final_epsilon > dp_config.target_epsilon) if dp_config.enabled else False,
    ...
    with torch.no_grad():
        for i in range(len(test_x)):
            ...
            hidden = model.encoder(data)
            node_reg = model.forward_regressor(hidden)
            preds_list.append(node_reg.cpu().numpy())
    ...
    mae_overall = float(np.mean(np.abs(preds_flat - targets_flat)))
    rmse_overall = float(np.sqrt(np.mean((preds_flat - targets_flat) ** 2)))

    delayed_mask = targets_flat >= delay_threshold
    if delayed_mask.sum() > 0:
        mae_delayed = float(np.mean(np.abs(preds_flat[delayed_mask] - targets_flat[delayed_mask])))
        rmse_delayed = float(np.sqrt(np.mean((preds_flat[delayed_mask] - targets_flat[delayed_mask]) ** 2)))
    else:
        mae_delayed, rmse_delayed = 0.0, 0.0

    nondelayed_mask = (targets_flat >= 1.0) & (targets_flat < delay_threshold)
    if nondelayed_mask.sum() > 0:
        mae_nondelayed = float(np.mean(np.abs(preds_flat[nondelayed_mask] - targets_flat[nondelayed_mask])))
        rmse_nondelayed = float(np.sqrt(np.mean((preds_flat[nondelayed_mask] - targets_flat[nondelayed_mask]) ** 2)))
    else:
        mae_nondelayed, rmse_nondelayed = 0.0, 0.0
    ...
    print(f"\nREGRESSION (non-delayed 1-{delay_threshold} min):")
    ...
    if VISUALIZATION_AVAILABLE:
        try:
            ...
            print("\n✓ Regression time-series visualization saved")
        except Exception as e:
            print(f"\n[VISUALIZATION] Skipped (error): {e}")

    print("\nPRIVACY BUDGET:")
    print(f"  Target ε: {dp_config.target_epsilon:.3f}")
    print(f"  Final ε: {final_epsilon:.3f}" if dp_config.enabled else "  DP disabled")
    if dp_config.enabled:
        print(f"  Final δ: {final_delta:.2e}")

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    sigma_str = f"sigma{dp_config.noise_multiplier:.2f}".replace(".", "_")
    history_csv = f"kan_gat_dp_regression_history_{sigma_str}_{timestamp}.csv"
    summary_csv = f"kan_gat_dp_regression_summary_{sigma_str}_{timestamp}.csv"
    ...
    with open(summary_csv, "w", newline="") as f:
        ...
        summary = {
            ...
            "training_time_minutes": float(train_time_seconds) / 60.0,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Single-pass DP-SGD regression for KAN-GAT (no stages)")
    ...
    parser.add_argument('--delayed_weight', type=float, default=3.0, help='Weight for targets >= delay_threshold (default: 3.0)')
    parser.add_argument('--model_path', type=str, default='kan_gat_dp_regression.pth')
    parser.add_argument(
        '--checkpoint_dir',
        type=str,
        default='auto',
...
def main() -> None:
    ...
    dp_enabled = bool(args.dp) and (not bool(args.no_dp))
    ...
    if dp_enabled:
        ...
        print(f"  Expected total steps: {total_steps} ({steps_per_epoch} steps/epoch × {args.epochs} epochs)")
    ...
    dp_config = DPConfig(
        ...
        sample_rate=sample_rate if dp_enabled else args.sample_rate,
    ...
    combined_history, accountant, train_time = train_regression_with_dp(
        model,
        train_x,
        train_y_reg,
        val_x,
    ...
    if dp_config.enabled:
        final_epsilon = accountant.get_epsilon(dp_config.target_delta)
    ...
    if args.model_path == 'kan_gat_dp_regression.pth':
        args.model_path = f"kan_gat_dp_regression_{sigma_str}_{timestamp}.pth"
    
    print(f"\nOutput model will be saved to: {args.model_path}")
    
    final_regression_evaluation(
        model,
        edge_indices,
        device,
        scaler,
