    ...
    def compute_per_sample_gradients(
        ...
        for i in range(batch_x.shape[0]):
            ...
            if is_classification:
                hidden, node_logits = self.model.forward_classifier(data)
                ...
                loss = loss_fn(graph_logit, graph_target, hidden=graph_hidden, **loss_kwargs)
            else:
                _, node_reg = self.model(data)
                graph_reg = aggregate_node_to_graph(node_reg)
                graph_target = ensure_graph_level_target(batch_y[i])
                mask = (graph_target >= 0).float()
                loss = loss_fn(graph_reg * mask, graph_target * mask, **loss_kwargs)
...
def aggregate_node_to_graph(node_features: torch.Tensor) -> torch.Tensor:
    """Aggregate node-level features to graph-level via max pooling."""
    # Use max pooling to favor strong positive evidence while being robust to class imbalance
...
def binary_focal_loss_logits(
    ...
    gamma: float,
    alpha_balance: Optional[float] = None,
) -> torch.Tensor:
    """Stable focal loss on logits with optional class balancing."""
...
def sse_loss(logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
    """Per-example sum-squared-error between logits and one-hot labels."""
    return 0.5 * ((logits - targets) ** 2).mean()


def activation_l2_penalty(
    hidden_states: Optional[torch.Tensor],
    device: Optional[torch.device] = None,
) -> torch.Tensor:
    """L2 penalty on intermediate activations (graph-level)."""
    if hidden_states is None:
        dev = device if device is not None else torch.device("cpu")
        return torch.tensor(0.0, device=dev)
    h = hidden_states
    dim = max(h.shape[-1], 1)
    return (h.pow(2).sum(dim=-1).mean()) / dim
...
def curriculum_dp_loss(
    ...
    *,
    ...
    """Curriculum loss combining SSE, focal, and activation regularizer."""
...
def train_stage1_with_dp(
    ...
    pos_weight: float,
    stage1_loss: str,
    stage1_huber_delta: float,
    stage1_gamma: float,
    stage1_beta: float,
    stage1_alpha_epoch: float,
    ...
    loss_mode = stage1_loss.lower()
    if loss_mode == "huber":
        huber = nn.HuberLoss(reduction="none", delta=stage1_huber_delta)

        def cls_loss_fn(logits: torch.Tensor, targets: torch.Tensor, hidden: Optional[torch.Tensor] = None, **_: Any) -> torch.Tensor:
            probs = torch.sigmoid(logits)
            loss_elem = huber(probs, targets)
            return loss_elem.mean()

        print(f"✓ Stage1 loss: Huber (delta={stage1_huber_delta})")
        ...
        def cls_loss_fn(
            ...
            gamma: float = stage1_gamma,
            alpha_balance: Optional[float] = None,
            **_: Any,
        ) -> torch.Tensor:
            if alpha_value is None:
                raise ValueError("alpha_value is required for dp_combo loss")
        ...
        print(
            f"✓ Stage1 loss: DP combo (gamma={stage1_gamma}, beta={stage1_beta}, alpha_epoch={stage1_alpha_epoch})"
        )
    else:
        ...
        print("✓ Stage1 loss: BCEWithLogits")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        epoch_losses = []
        current_alpha = None
        loss_kwargs_epoch: Dict[str, Any] = {}
        if loss_mode == "dp_combo":
            current_alpha = torch.sigmoid(
                torch.tensor(epoch - stage1_alpha_epoch, device=device, dtype=torch.float32)
            )
            loss_kwargs_epoch = {
                'alpha_value': current_alpha,
                'beta': stage1_beta,
                'gamma': stage1_gamma,
                'alpha_balance': pos_weight if use_class_balance else None,
            }
        ...
        for start_idx in range(0, num_samples, batch_size):
            ...
            if dp_config.enabled:
                ...
                per_sample_grads = clipper.compute_per_sample_gradients(
                    batch_x, batch_y, edge_indices, cls_loss_fn, is_classification=True, loss_kwargs=loss_kwargs_epoch
                )
        ...
        history.append({
            ...
            'alpha_value': float(current_alpha) if current_alpha is not None else None,
        })
        
        eps_str = f"ε: {current_epsilon:.3f}/{dp_config.target_epsilon}" if dp_config.enabled else "No DP"
        alpha_msg = f" | alpha: {current_alpha.item():.3f}" if current_alpha is not None else ""
        print(
            f"Epoch {epoch}/{epochs} | Loss: {history[-1]['train_loss']:.4f} | "
            f"Val F1: {val_metrics['f1']:.4f} | {eps_str} | Time: {epoch_time:.2f}s{alpha_msg}"
        )
...
def train_stage2_with_dp(
    ...
    if stage2_loss.lower() == "huber":
        huber = nn.HuberLoss(reduction="none", delta=stage2_huber_delta)

        def reg_loss_masked(pred: torch.Tensor, target: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
            loss_elem = huber(pred, target)
            if mask is not None:
                loss_elem = loss_elem * mask
                denom = mask.sum().clamp_min(1e-8)
            else:
                denom = torch.tensor(loss_elem.numel(), device=loss_elem.device, dtype=loss_elem.dtype)
            return loss_elem.sum() / denom

        def reg_loss_unmasked(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
            return huber(pred, target).mean()

        print(f"✓ Stage2 loss: Huber (delta={stage2_huber_delta})")
    else:
        def reg_loss_masked(pred: torch.Tensor, target: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
            if mask is not None:
                denom = mask.sum().clamp_min(1e-8)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for start_idx in range(0, num_samples, batch_size):
            ...
            if dp_config.enabled:
                per_sample_grads = clipper.compute_per_sample_gradients(
                    batch_x, batch_y_reg, edge_indices, reg_loss_unmasked, is_classification=False
                )
                noisy_grads = clipper.add_noise_to_gradients(
                    per_sample_grads,
                    dp_config.noise_multiplier,
                ...
                with torch.no_grad():
                    ...
                    loss = reg_loss_masked(reg_preds, reg_targets, mask)
        ...
        with torch.no_grad():
            for i in range(len(val_x)):
                ...
                loss = reg_loss_masked(graph_reg.cpu(), graph_target, mask)
...
def train_stage3_with_dp(
    ...
    # Keep encoder and classifier frozen (regressor-only fine-tuning)
    for param in model.encoder.parameters():
        param.requires_grad = False
    for param in model.classifier.parameters():
        param.requires_grad = False
    ...
    reg_loss_fn = nn.HuberLoss(reduction="none", delta=2.0)
    ...
    # When to start auxiliary loss on delayed samples (encoder stays frozen)
    aux_loss_start_epoch = 1
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for start_idx in range(0, num_samples, batch_size):
            ...
            if num_nondelayed > 0:
                ...
                # Optional auxiliary loss on delayed elements after warmup
                delayed_mask = (targets_denorm.abs() >= delay_threshold).float()
                num_delayed = delayed_mask.sum()

                if num_delayed > 0 and epoch >= aux_loss_start_epoch:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--stage1_loss', type=str, default='dp_combo', choices=['bce', 'huber', 'dp_combo'])
    parser.add_argument('--stage1_huber_delta', type=float, default=1.0)
    parser.add_argument('--stage1_gamma', type=float, default=2.0, help='Focusing parameter for focal component (Stage1)')
    parser.add_argument('--stage1_beta', type=float, default=2.0, help='Scaling for activation regularizer (Stage1)')
    parser.add_argument('--stage1_alpha_epoch', type=float, default=1.0, help='Epoch threshold for alpha curriculum (Stage1)')
    parser.add_argument('--stage1_use_class_balance', dest='stage1_use_class_balance', action='store_true', help='Enable class balance weight inside focal term')
    parser.add_argument('--stage1_no_class_balance', dest='stage1_use_class_balance', action='store_false', help='Disable class balance weight inside focal term')
    parser.set_defaults(stage1_use_class_balance=True)
    parser.add_argument('--stage2_loss', type=str, default='mse', choices=['mse', 'huber'])
    parser.add_argument('--stage2_huber_delta', type=float, default=1.0)
...
def main() -> None:
    ...
    history_s1, accountant_s1, stage1_time = train_stage1_with_dp(
        ...
        pos_weight, args.stage1_loss, args.stage1_huber_delta,
        args.stage1_gamma, args.stage1_beta, args.stage1_alpha_epoch,
        args.stage1_use_class_balance,
        args.patience, dp_config, args.batch_size,
    )
    ...
    history_s2, accountant_s2, stage2_time = train_stage2_with_dp(
        ...
        args.stage2_loss, args.stage2_huber_delta,
