...
def main():
    ...
    print(f"\n{'='*95}")
    print(f"  TEST SET Soft Gating Sweep (Softness `k` at th=0.5)")
    print(f"{'='*95}")
    ...
    for k in thresholds:
        ...
        t_mae_del, _ = compute_regression_metrics(soft_preds[delayed_mask], test_targets_raw[delayed_mask])
        t_mae_nd, _ = compute_regression_metrics(soft_preds[nondelayed_mask], test_targets_raw[nondelayed_mask])

        if t_mae < best_mae: 
            best_mae, best_k = t_mae, k
            
        print(f"  {k:10.1f} | {t_mae:9.2f} {t_rmse:10.2f} | {t_mae_del:9.2f} {t_mae_nd:9.2f}")

    print(f"\n  >>> Best TEST MAE: {best_mae:.2f} min at softness k = {best_k:.1f}")
    ...
    print(f"\n{'='*95}\n  FINAL PERFORMANCE (TEST SET) at Best Softness k={best_k:.1f}\n{'='*95}")
