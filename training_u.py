...
def main():
    ...
    MAE_list = []
    batch_index = list(range(training_data.shape[1] - (args.in_len + args.out_len))) #create sliding window
    #len of batch_index = 1000 - (12 + 12) = 976
    
    val_index = list(range(val_data.shape[1] - (args.in_len + args.out_len)))

#val_index[i] + args.in_len: Starting point - right after the input sequence ends, al_index[i] + args.in_len + args.out_len: Ending point - covers the entire output prediction horizon
#np.expand_dims(..., axis=0): Adds a batch dimension, transforming from [nodes, output_length, features] to [1, nodes, output_length, features]

    label = []
    for i in range(len(val_index)):#constructs the ground truth labels for validation
    ...
    for ep in range(1, 1+args.episode):
        ...
        for j in range(len(batch_index) // args.batch - 1):
            ...
            trainx= trainx.permute(0, 3, 1, 2) ## Reshape to [batch, features, airports, time]
