...
parser.add_argument('--noise_multiplier', type=float, default=1, help='sigma; used if target_epsilon <= 0')
...
# LSTMModel class for DP training (Opacus compatible)
class LSTMModel(nn.Module):
    def __init__(self, in_channels=2, out_channels=2, hidden_size=64, num_layers=2, dropout=0.0, num_nodes=70):
        ...
        # Initialize with default input size (features * nodes)
        ...
        # FC layer should output (out_channels * num_nodes) to match target shape
    ...
    def _rebuild_layers_if_needed(self, features, nodes):
        ...
        if required_input_size != self._initialized_input_size or nodes != self.num_nodes:
            # Rebuild layers with correct input size
            self.lstm = DPLSTM(
    ...
    def forward(self, x, ti=None, supports=None, to=None, w=None):
        # x: (features, nodes, seq_len) or (batch, features, nodes, seq_len)
        ...
        out = self.fc(out)     # (batch, seq_len, out_channels * nodes)
        
        # Reshape back to match expected output format
        if len(original_shape) == 3:
            f, n, s = original_shape
            out = out.view(-1, s, self.out_channels, n).permute(0, 2, 3, 1)  # (batch, out_channels, nodes, seq_len)
            return out.squeeze(0)  # Remove batch dim if input was 3D
        else:
            b, f, n, s = original_shape
            out = out.view(b, s, self.out_channels, n).permute(0, 2, 3, 1)  # (batch, out_channels, nodes, seq_len)
            return out
