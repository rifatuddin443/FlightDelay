...
parser.add_argument('--episode',type=int,default=13,help='training episodes')
...
parser.add_argument('--noise_multiplier', type=float, default=0.8, help='sigma; used if target_epsilon <= 0')
...
def main():
    ...
    for ep in range(1, 1 + args.episode):
        ...
        if mean_mae == min(MAE_LIST := MAE_list):  # store best
            best_model = copy.deepcopy(model.state_dict())
