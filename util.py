...
#     mask /= torch.mean(mask, dim=tuple(range(1, mask.dim())), keepdim=True)  # Normalize per sample
#     mask = torch.where(torch.isnan(mask), torch.zeros_like(mask), mask)
...
#     loss_per_sample = loss.mean(dim=tuple(range(1, loss.dim())))

#     return loss_per_sample.mean()  # Mean over batch samples for scalar loss
