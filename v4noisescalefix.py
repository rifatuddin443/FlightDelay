...
class MinMaxScaler:
    """Min-Max scaler that scales data to a uniform range [0, 1] without mean shifting.
    
    This scaler only applies scaling (division by range) without any centering/mean shifting.
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if epoch % 5 == 0 or epoch == 1:
            with torch.no_grad():
                ...
                print(f"    Target (scaled): min={train_y_reg[sample_idx].min().item():.3f}, max={train_y_reg[sample_idx].max().item():.3f}, mean={train_y_reg[sample_idx].mean().item():.3f}")
                mask_count = (train_y_cls[sample_idx] >= class_threshold).sum().item()
                total_count = train_y_cls[sample_idx].numel()
                print(f"    Mask coverage: {mask_count}/{total_count} ({100*mask_count/total_count:.1f}% masked)")
        
        # Validation in denormalized space (original scale in minutes)
        model.eval()
        val_losses = []
        val_maes = []
        with torch.no_grad():
            for i in range(len(val_x)):
                ...
                # Denormalize predictions and targets to original scale (minutes)
                ...
                # Per-channel masked MSE/MAE in denormalized space (minutes)
                se = (pred_denorm - target_denorm) ** 2
                ae = torch.abs(pred_denorm - target_denorm)
                
                num_mse = (se * mask).sum(dim=0)
                num_mae = (ae * mask).sum(dim=0)
                den = mask.sum(dim=0).clamp_min(1.0)
                
                loss = (num_mse / den).mean()  # RMSE will be sqrt(loss)
                mae = (num_mae / den).mean()
                
                val_losses.append(loss.item())
                val_maes.append(mae.item())
        
        val_loss = np.mean(val_losses)  # MSE in minutes^2
        val_rmse = np.sqrt(val_loss)  # RMSE in minutes
        val_mae = np.mean(val_maes)  # MAE in minutes
        epoch_time = time.time() - epoch_start_time
        ...
        print(
            ...
            f"Val MAE: {val_mae:.2f} min | Val RMSE: {val_rmse:.2f} min | {eps_str} | Time: {epoch_time:.2f}s"
        )
        
        if val_mae < best_val_loss:  # Use MAE for early stopping (more interpretable)
...
def train_stage3_with_dp(
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        with torch.no_grad():
            for i in range(len(val_x)):
                ...
                if num_nondelayed > 0:
                    ...
                    ae = torch.abs(pred_denorm - target_denorm) * element_mask
                    mae_val_ch = ae.sum(dim=0) / num_nondelayed_per_ch.clamp_min(1.0)
                    mae_val = mae_val_ch.mean()
                    
                    val_losses.append(loss_val.item())
                    val_maes.append(mae_val.item())
                    val_nondelayed += num_nondelayed.item()
                    val_total += float(element_mask.numel())

        val_loss = float(np.mean(val_losses)) if val_losses else 0.0
        val_rmse = float(np.sqrt(val_loss)) if val_loss > 0 else 0.0
        val_mae = float(np.mean(val_maes)) if val_maes else 0.0
        epoch_time = time.time() - epoch_start_time
        ...
        # Calculate validation nondelayed ratio
        ...
        print(
            f"Epoch {epoch}/{epochs} | Loss: {history[-1]['train_loss']:.4f} | "
            f"Val MAE: {val_mae:.2f} min | Val RMSE: {val_rmse:.2f} min | Non-delayed: {total_nondelayed:.0f} "
            f"({nondelayed_ratio*100:.1f}%) | Val ND: {val_nondelayed:.0f} "
            f"({val_nondelayed_ratio*100:.1f}%) | {eps_str} | Time: {epoch_time:.2f}s"
        )

        if val_mae < best_val_loss and val_nondelayed > 0:  # Use MAE for early stopping
            best_val_loss = val_mae
            best_state = {
                "encoder": model.encoder.state_dict(),
...
def final_evaluation(
    ...
    if scaler is not None:
        ...
        print(f"    Scaler range: min={scaler.min}, max={scaler.max}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--noise_multiplier', type=float, default=1.2, help='Fixed noise multiplier for DP-SGD (lower=less noise, less privacy)')
...
def main() -> None:
    ...
    # Replace StandardScaler with MinMaxScaler (no mean shifting, only range scaling)
    print("\n[SCALING] Replacing StandardScaler with MinMaxScaler (no mean shift)...")
    
    # 1. Scale delay data (targets)
    print("\n  [1/2] Scaling delay targets...")
    all_raw_delays = np.concatenate([
        train_raw.reshape(-1, train_raw.shape[-1]),
        val_raw.reshape(-1, val_raw.shape[-1]),
        test_raw.reshape(-1, test_raw.shape[-1])
    ], axis=0)
    
    # Check for and handle NaN values before fitting scaler
    nan_mask = np.isnan(all_raw_delays).any(axis=1)
    if nan_mask.any():
        nan_count = nan_mask.sum()
        print(f"    WARNING: Found {nan_count} samples with NaN values ({nan_count/len(all_raw_delays)*100:.2f}%)")
        print(f"    Filtering out NaN samples before fitting scaler...")
        all_raw_delays_clean = all_raw_delays[~nan_mask]
        print(f"    Clean samples for fitting: {len(all_raw_delays_clean)} / {len(all_raw_delays)}")
    else:
        all_raw_delays_clean = all_raw_delays
        print(f"    No NaN values detected in delay data ✓")
    
    # Create and fit MinMaxScaler for delays using clean data
    scaler = MinMaxScaler()
    scaler.fit(all_raw_delays_clean)
    
    # Re-scale the delay data using MinMaxScaler
    train_delay_scaled = torch.from_numpy(
        scaler.transform(train_raw.reshape(-1, train_raw.shape[-1])).reshape(train_raw.shape)
    ).float()
    val_delay_scaled = torch.from_numpy(
        scaler.transform(val_raw.reshape(-1, val_raw.shape[-1])).reshape(val_raw.shape)
    ).float()
    test_delay_scaled = torch.from_numpy(
        scaler.transform(test_raw.reshape(-1, test_raw.shape[-1])).reshape(test_raw.shape)
    ).float()
    
    print(f"    Delay scaler range: [{scaler.min}, {scaler.max}]")
    print(f"    Scaled delays in range: [0, 1]")
    print(f"    ✓ Delay scaling complete")
    
    # 2. Scale input features (weather and time data)
    print("\n  [2/2] Scaling input features (weather + time)...")
    
    # Get all input features to fit the scaler
    all_inputs = np.concatenate([
        train_inputs.reshape(-1, train_inputs.shape[-1]),
        val_inputs.reshape(-1, val_inputs.shape[-1]),
        test_inputs.reshape(-1, test_inputs.shape[-1])
    ], axis=0)
    
    # Create and fit MinMaxScaler for input features
    input_scaler = MinMaxScaler()
    input_scaler.fit(all_inputs)
    
    # Re-scale the input features using MinMaxScaler
    train_inputs = torch.from_numpy(
        input_scaler.transform(train_inputs.reshape(-1, train_inputs.shape[-1])).reshape(train_inputs.shape)
    ).float()
    val_inputs = torch.from_numpy(
        input_scaler.transform(val_inputs.reshape(-1, val_inputs.shape[-1])).reshape(val_inputs.shape)
    ).float()
    test_inputs = torch.from_numpy(
        input_scaler.transform(test_inputs.reshape(-1, test_inputs.shape[-1])).reshape(test_inputs.shape)
    ).float()
    
    print(f"    Input scaler range: [{input_scaler.min}, {input_scaler.max}]")
    print(f"    Scaled inputs in range: [0, 1]")
    print(f"    Feature dimension: {train_inputs.shape[-1]}")
    print(f"    ✓ Input scaling complete")
    
    print("\n  ✓ All data scaled to uniform range [0, 1] without mean shifting")
    ...
    print(f"  Scaler type: MinMaxScaler (range-only scaling, no mean shift)")
    print(f"  Scaler min: {scaler.min}")
    print(f"  Scaler max: {scaler.max}")
    print(f"  Scaler range: {scaler.range}")
