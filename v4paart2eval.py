...
# Import the correct model architecture from v4part2.py
try:
    from v4part2 import (
        SequentialTwoStagePredictor,
        classification_metrics_per_channel,
        build_sequences_temporal,
    )
    MODEL_V4PART2_AVAILABLE = True
except ImportError:
    print("Warning: Could not import from v4part2.py. Using fallback.")
    from classifykat import SequentialTwoStagePredictor  # noqa: E402
    MODEL_V4PART2_AVAILABLE = False
    
    # Fallback: define a simple wrapper for per-channel metrics
    ...
    build_sequences_temporal = build_sequences_node_level
...
def find_latest_model(pattern: str = "kan_gat_dp_three_stage*.pth") -> str:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--hidden_channels", type=int, default=16, help="Hidden channels (must match training: v4part2.py uses 16)")
...
def main() -> None:
    ...
    # v4part2 uses feature_dim directly (not flattened), with seq_len as separate parameter
    in_channels = feature_dim
    seq_len = args.seq_len
    out_channels = delay_dim
    
    # Build test sequences using temporal-aware function
    if MODEL_V4PART2_AVAILABLE:
        print("[INFO] Using temporal build function from v4part2")
        build_fn = build_sequences_temporal
    else:
        print("[INFO] Using node-level build function (fallback)")
