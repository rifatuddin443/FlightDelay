"""Test script for v4noisescalefix.py models (single-horizon, DP, three-stage with MinMaxScaler).

Designed to evaluate checkpoints produced by v4noisescalefix.py, which trains
with a single forecast horizon, fixed Gaussian noise multiplier, Stage 3
fine-tune on flights under the delay threshold, and MinMaxScaler for uniform [0,1] range scaling.

Key differences from standard evaluation:
- Uses MinMaxScaler instead of StandardScaler
- Scales both inputs (weather+time) and targets (delays) to [0, 1] without mean shifting
- All metrics computed in denormalized space (original minutes)

Usage:
    # Auto-detect latest model
    python v4scalefix_eval.py --horizons 12

    # Evaluate specific model
    python v4scalefix_eval.py --model_path kan_gat_dp_three_stage_sigma1_20_20260106_143022.pth --horizons 12
    
    # With custom settings
    python v4scalefix_eval.py --horizons 12 --delay_threshold 5.0 --class_threshold 0.5 --data_source cdata
"""
...
class MinMaxScaler:
    ...
    def inverse_transform(self, X_scaled):
        """Transform scaled data back to original range."""
        if self.min is None or self.max is None:
            raise ValueError("Scaler not fitted yet. Call fit() first.")
...
def _load_three_stage_model(
    ...
    if all(key in checkpoint for key in ("encoder", "classifier", "regressor")):
        try:
            ...
            print(f"     - Regressor: trained in Stage 2 (delayed) + Stage 3 (non-delayed)")
...
def find_latest_model(pattern: str = "kan_gat_dp_three_stage_sigma*.pth") -> str:
    ...
    if not model_files:
        raise FileNotFoundError(
            ...
            f"Please train a model first using v4noisescalefix.py or specify --model_path"
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Test three-stage DP models (v4noisescalefix.py) with MinMaxScaler and separate horizon-wise evaluation.",
    )
    parser.add_argument("--model_path",       type=str,        default="auto",
        help="Path to the three-stage trained checkpoint. Use 'auto' to find the latest kan_gat_dp_three_stage_*.pth file (trained with v4noisescalefix.py).",
    )
    ...
    parser.add_argument(
        ...
        default=[12],
        choices=[3, 6, 12, 24],
        help="Single forecast horizon (must match v4noisescalefix training run).",
    )
    ...
    parser.add_argument("--hidden_channels", type=int, default=64, help="Hidden channels (must match training: v4noisescalefix uses 64)")
    parser.add_argument("--summary_csv", type=str, default="v4_minmax_test_summary.csv")
    parser.add_argument("--predictions_csv", type=str, default="v4_minmax_test_predictions.csv")
    parser.add_argument("--results_table_csv", type=str, default="v4_minmax_results_table.csv")
...
def main() -> None:
    ...
    # Replace StandardScaler with MinMaxScaler (matching v4noisescalefix.py)
    ...
    # Create and fit MinMaxScaler for delays
    scaler = MinMaxScaler()
    scaler.fit(all_raw_delays)
    ...
    if args.summary_csv == "v4_minmax_test_summary.csv":
        args.summary_csv = f"v4_minmax_test_summary_{eps_str}_{timestamp}.csv"
    if args.predictions_csv == "v4_minmax_test_predictions.csv":
        args.predictions_csv = f"v4_minmax_test_predictions_{eps_str}_{timestamp}.csv"
    if args.results_table_csv == "v4_minmax_results_table.csv":
        args.results_table_csv = f"v4_minmax_results_table_{eps_str}_{timestamp}.csv"
